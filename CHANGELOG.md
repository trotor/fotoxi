...
- **Resolving a duplicate group that no longer exists now 404s instead of reporting a silent success** (#52) — `resolve_duplicate_group` always returned success even when no group members matched the requested id. That happens because grouping deletes and re-creates every unresolved group on each index run, giving them new ids, so a user triaging duplicates while an index finishes could get success toasts for choices that were never saved. It now returns `False` when nothing matched, and the route raises a 404 instead of the previous `{"status": "resolved"}`. A partially-valid request still resolves its real members and returns 200. (`routes.py`, `queries.py`)
...
- **Duplicate grouping now honours a Stop request, and no longer finishes at 12%** (#55) — grouping ignored the stop signal entirely, including mid-write, so a Stop partway through a run could leave the `duplicate_groups` table half-rewritten. It now checks the stop flag before loading images, before the CPU-bound grouping pass, and again before the write phase begins, so a stopped run makes no database changes. Progress previously counted images in the total but groups in the processed count, so the bar ended at roughly 12%; the write phase now counts groups on both sides and reaches 100%. The load and comparison passes still report 0% while they run — that half is tracked separately. (`orchestrator.py`)
